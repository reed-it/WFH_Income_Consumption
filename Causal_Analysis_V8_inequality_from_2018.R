# Difference-in-Differences Analysis: WFH Impact on Income and Consumption Inequality
...
# Load the data
data <- read_csv("LCFS_DiD_data_aggregate_from_2018.csv")
...
# Model 1: Basic DiD for Income inequality
# Using the pre-calculated did_effect variable
model_income_basic <- feols(log_var_income ~ did_effect, data = data, vcov = "hetero")

# Model 2: DiD for Income inequality with year fixed effects
model_income_fe <- feols(log_var_income ~ did_effect + year_2018 + year_2019 + 
                           year_2020 + year_2021 + year_2022, 
                         data = data, vcov = "hetero")

# Model 3: Manual DiD construction for Income inequality (alternative specification)
model_income_manual <- feols(log_var_income ~ treatment_1 * WfhCor_1, data = data, vcov = "hetero")

# Model 4: Manual DiD with year fixed effects for Income Inequality
model_income_manual_fe <- feols(log_var_income ~ treatment_1 * WfhCor_1 + year_2018 + 
...
# Model 5: Basic DiD for Consumption inequality
model_consumption_basic <- feols(log_var_consumption ~ did_effect, data = data, vcov = "hetero")

# Model 6: DiD for Consumption inequality with year fixed effects
model_consumption_fe <- feols(log_var_consumption ~ did_effect  + year_2018 + 
                                year_2019 + year_2020 + year_2021 + year_2022, 
                              data = data, vcov = "hetero")

# Model 7: Manual DiD construction for Consumption inequality
model_consumption_manual <- feols(log_var_consumption ~ treatment_1 * WfhCor_1, data = data, vcov = "hetero")

# Model 8: Manual DiD with year fixed effects consumption inequality
model_consumption_manual_fe <- feols(log_var_consumption ~ treatment_1 * WfhCor_1   + 
...
modelsummary(income_models, 
             title = "Difference-in-Differences: Impact of WFH on Income Inequality",
...
modelsummary(consumption_models, 
             title = "Difference-in-Differences: Impact of WFH on Consumption Inequality",
             stars = c('*' = 0.1, '**' = 0.05, '***' = 0.01),
             gof_map = c("nobs", "r.squared", "adj.r.squared"))
...
yearly_means <- data_long %>%
  ...
  summarise(
    mean_income = mean(log_var_income, na.rm = TRUE),
    mean_consumption = mean(log_var_consumption, na.rm = TRUE),
    n = n(),
    se_income = sd(log_var_income, na.rm = TRUE) / sqrt(n()),
    se_consumption = sd(log_var_consumption, na.rm = TRUE) / sqrt(n()),
    .groups = 'drop'
  )
...
p_consumption_trends <- ggplot(yearly_means, aes(x = year_num, y = mean_consumption, 
  ...
  annotate("text", x = treatment_years, y = max(yearly_means$mean_consumption) * 0.95, 
           label = "WFH Policy\nImplemented", hjust = -0.1, color = "red") +
  labs(title = "Consumption Inequality Over Time",
       subtitle = "Pre-treatment trends should be parallel between WFH-eligible and non-eligible groups",
       x = "Year",
       y = "Log Var. Consumption",
       color = "Treatment Group") +
  scale_color_manual(values = c("0" = "red", "1" = "blue"),
                     labels = c("Non-WFH Eligible", "WFH Eligible")) +
  theme_minimal() +
...
trend_test_income <- feols(log_var_income ~ treatment_group * time_trend + factor(year_num), 
                           data = pre_treatment_data, vcov = "hetero")

# Consumption trend test  
trend_test_consumption <- feols(log_var_consumption ~ treatment_group * time_trend + factor(year_num), 
                                data = pre_treatment_data, vcov = "hetero")

cat("Pre-treatment trend test for INCOME Inequality:\n")
print(summary(trend_test_income))

cat("Pre-treatment trend test for CONSUMPTION Inequality:\n")
...
if(all(data$income > 0, na.rm = TRUE) & all(data$derived_consumption > 0, na.rm = TRUE)) {
  ...
  model_log_income <- feols(log_var_income ~ did_effect + year_2018 + 
                              year_2019 + year_2020 + year_2021 + year_2022, 
                            data = data, vcov = "hetero")
  
  model_log_consumption <- feols(log_var_consumption ~ did_effect +  
                                   year_2018 + year_2019 + year_2020 + year_2021 + year_2022, 
                                 data = data, vcov = "hetero")
